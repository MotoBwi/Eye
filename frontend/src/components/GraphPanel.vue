...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Changed from simulating to not simulating, show end hint
...
const renderGraph = () => {
  ...
  // Count edge count between node pairs, collect self-loop edges
  tempEdges.forEach(e => {
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add centripetal force to gather isolated node groups to center area
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw a circular arc from node and return
      const loopRadius = 30
      // Start from right side of node, circle around and return
      const x1 = sx + 8  // Start point offset
      const y1 = sy - 4
      const x2 = sx + 8  // End point offset
      const y2 = sy + 4
      // Use circular arc to draw self-loop (sweep-flag=1 clockwise)
    ...
    // Offset perpendicular to connection direction, calculated based on distance ratio, ensure curve is clearly visible
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Only restart simulation when first detected real drag
          d._isDragging = true
