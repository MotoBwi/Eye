...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect and log round changes for each platform separately
      ...
      // Additional check: if backend hasn't updated runner_status yet but platforms report completed
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // New actions are appended at bottom
    }
  } catch (err) {
...
onMounted(() => {
  addLog('Step3 simulation running initialized')
  if (props.simulationId) {
    doStartSimulation()
  }
})
