  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, only record state, wait for animation to finish before processing
        ...
        // Use debounce to delay state switch, prevent fast flickering
        // Short delay when expanding (50ms), longer delay when collapsing (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if pending state still needs to be executed (may have been overridden by subsequent scroll)
          ...
          setTimeout(() => {
            ...
            // After animation ends, check if there is new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay a small time before executing, avoid too fast switch
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, viewport bottom contracts upward, need to scroll more to trigger expand
