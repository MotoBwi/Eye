...
Run Twitter and Reddit simulations simultaneously, read the same config file

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- After simulation completes, don't close environment immediately, enter wait-for-command mode
...
# ============================================================
# Solve Windows encoding issue: Set UTF-8 before all imports
# This is to fix the issue of OASIS third-party library not specifying encoding when reading files
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output stream to UTF-8 (solve console Chinese garbled)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This can fix the issue of third-party libraries (like OASIS) not specifying encoding when reading files
        """
...
# Script fixed in backend/scripts/ directory
...
def disable_oasis_logging():
    """
    Disable OASIS library detailed log output
    OASIS logs are too verbose (record each agent's observations and actions), we use our own action_logger
    """
...
# Twitter available actions (excluding INTERVIEW, INTERVIEW can only be manually triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excluding INTERVIEW, INTERVIEW can only be manually triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    Manage two platform environments, handle Interview commands
    """
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get specified platform environment and agent_graph
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(command_id, "failed", error=f"Unknown command types: {command_type}")
            return True
...
# Non-core action types to filter out（These actions have low analysis value）
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This way can display real entity names in actions.jsonl，instead of codes like
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from Database and supplement complete context info

    Args:
        db_path: Database file path
        last_rowid: last read max rowid value (Use rowid instead of created_at, because different platforms have different created_at formats)
    ...
    try:
        ...
        # This can avoid created_at format differences (Twitter uses integer, Reddit uses datetime string)
        cursor.execute("""
...
def _enrich_action_context(
    ...
    try:
        ...
        # Quote post: supplement original post content, author and quoted comment
            ...
            # Get quoted post's comment content（quote_content）
            if new_post_id:
                cursor.execute("""
        ...
        # Supplement context failure doesn't affect main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Support dual LLM config，used to speed up parallel simulation：
    ...
    If boost LLM configured, parallel simulation can let different platforms use different API providers，Improve concurrent capability。
...
def get_active_agents_for_round(
    ...
    """Determine which Agents to activate in this round based on time and config"""
...
async def run_twitter_simulation(
    ...
    # Record start (Initial event phase)
    if action_logger:
    ...
    for round_num in range(total_rounds):
        ...
        # Record start regardless of whether there are active agents
        if action_logger:
            action_logger.log_round_start(round_num + 1, simulated_hour)
        
        if not active_agents:
            # Record end even when there are no active agents (actions_count=0)
            if action_logger:
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Max simulation rounds（Optional, used to truncate too long simulation）'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close simulation environment immediately after completion, do not enter wait for command mode'
    )
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop complete! Total time elapsed: {total_elapsed:.1f}seconds")
    
    # Whether to enter wait for command mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Enter wait for command mode - environment keeps running")
...
def setup_signal_handlers(loop=None):
    """
    Set signal handler, ensure proper exit when receiving SIGTERM/SIGINT
    ...
    2. Give program chance to clean up resources normally (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        # Do not directly sys.exit(), let asyncio loop exit normally and clean up resources
        # Only force exit if signal received multiple times
        else:
            print("Force exit...")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Cleanup multiprocessing resource tracker (prevent warnings on exit)
        try:
