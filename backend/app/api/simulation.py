...
def prepare_simulation():
    ...
    这是一个耗时操作，接口会立即Returnstask_id，
    使用 GET /api/simulation/prepare/status 查询进度
    
    特性：
    - 自动检测已完成的准备工作，避免重复生成
    - 如果已准备完成，直接Returns已有结果
    ...
    4. LLM智能Generate simulation config（带重试机制）
    5. 保存配置文件和预设脚本
    ...
    Request（JSON）：
        {
            ...
            "parallel_profile_count": 5,                  // Optional，并行生成人设数量，Default5
    ...
    try:
        ...
        if not force_regenerate:
            ...
            if is_prepared:
                logger.info(f"模拟 {simulation_id} Already prepared, skip duplicate generation")
                return jsonify({
                    "success": True,
                    "data": {
        ...
        # ========== 同步获取Entity count（在后台任务启动前） ==========
        # 这样前端在调用prepare后立即就能获取到预期Agent总数
        try:
            logger.info(f"同步获取Entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # 快速读取实体（不需要边信息，只统计数量）
            ...
            # 保存Entity count到状态（供前端立即获取）
            state.entities_count = filtered_preview.filtered_count
            state.entity_types = list(filtered_preview.entity_types)
            logger.info(f"预期Entity count: {filtered_preview.filtered_count}, Types: {filtered_preview.entity_types}")
        except Exception as e:
...
def get_prepare_status():
    ...
    Request（JSON）：
        {
            "task_id": "task_xxxx",          // Optional，prepareReturns的task_id
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    如果有多个则Returns最新的（按 created_at 排序）
...
def get_simulation_history():
    ...
    用于首页历史项目展示，Returns包含项目名称、描述等丰富信息的模拟列表
    
    Query参数：
        limit: Returns数量限制（Default20）
...
def get_simulation_profiles(simulation_id: str):
    ...
    Query参数：
        platform: 平台Types（reddit/twitter，Defaultreddit）
    """
    try:
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"实时Failed to get Profile: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config(simulation_id: str):
    ...
    Returns包含：
        - time_config: 时间配置（模拟时长、轮次、高峰/Low period）
        - agent_configs: 每个Agent的活动配置（活跃度、Post frequency、Stance等）
...
def start_simulation():
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"无效的平台Types: {platform}，Optional: twitter/reddit/parallel"
            }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Query参数：
        platform: 过滤平台（twitter/reddit，Optional）
...
def get_simulation_posts(simulation_id: str):
    ...
    Query参数：
        ...
        limit: Returns数量（Default50）
        offset: 偏移量
    
    Returns帖子列表（从SQLite数据库读取）
    """
    try:
        platform = request.args.get('platform', 'reddit')
    ...
    Request（JSON）：
        {
            ...
            "platform": "twitter",             // Optional，指定平台（twitter/reddit）
                                               // 不指定时：双平台模拟同时采访两个平台
            "timeout": 60                      // Optional，超时时间（秒），Default60
        }

    Returns（不指定platform，双平台模式）：
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    Request（JSON）：
        {
            ...
            "platform": "reddit",              // Optional，Default平台（被每项的platform覆盖）
                                               // 不指定时：双平台模拟每个Agent同时采访两个平台
            "timeout": 120                     // Optional，超时时间（秒），Default120
        }
    ...
    Request（JSON）：
        {
            ...
            "platform": "reddit",                   // Optional，指定平台（twitter/reddit）
                                                    // 不指定时：双平台模拟每个Agent同时采访两个平台
            "timeout": 180                          // Optional，超时时间（秒），Default180
        }
...
def get_interview_history():
    ...
    Request（JSON）：
        {
            ...
            "platform": "reddit",          // Optional，平台Types（reddit/twitter）
                                           // 不指定则Returns两个平台的所有历史
            "agent_id": 0,                 // Optional，只获取该Agent的采访历史
            "limit": 100                   // Optional，Returns数量，Default100
        }
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # 不指定则Returns两个平台的历史
        agent_id = data.get('agent_id')
...
def close_simulation_env():
    ...
    Request（JSON）：
        {
            "simulation_id": "sim_xxxx",  // Required，模拟ID
            "timeout": 30                  // Optional，超时时间（秒），Default30
        }
