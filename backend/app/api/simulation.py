...
# Adding this prefix avoids Agent calling tools, directly responding with text
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match predefined entity types (Labels not just Entity)
...
def create_simulation():
    ...
    Note: max_rounds and other parameters are intelligently generated by LLM, no manual setup required
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # - running: running means preparation was already done
        # - completed: completed means preparation was already done
        # - stopped: stopped means preparation was already done
        # - failed: failed (but preparation was complete)
...
def prepare_simulation():
    ...
    Steps:
    1. Check if already has completed preparation
    2. Read and filter entities from Zep knowledge graph
    ...
    Returns:
        {
            ...
            "data": {
                "simulation_id": "sim_xxxx",
                "task_id": "task_xxxx",           // Returns when new task
                "status": "preparing|ready",
                "message": "Preparation task started|Already has completed preparation",
    ...
    try:
        ...
        if not force_regenerate:
            logger.debug(f"Check if simulation is already prepared...")
...
def get_prepare_status():
    ...
    2. Check if already has completed preparation by simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // Whether has completed preparation
                "prepare_info": {...}            // Detailed info when prepared
            }
        }
    """
    ...
    try:
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Already has completed preparation",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # Task doesn't exist, but if simulation_id provided, check if prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation(simulation_id: str):
    ...
    try:
        ...
        # If simulation is ready, add running instructions
...
def get_simulation_history():
    ...
    Used for homepage historical project display, returns simulation list with rich info like project name, description
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (may be still writing): {e}")
                profiles = []
        ...
        logger.error(f"Realtime failed to get Profile: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (may be still writing): {e}")
                config = None
...
def get_simulation_config(simulation_id: str):
    ...
    Returns includes:
        - time_config: Time config (simulation duration, rounds, peak/low period)
        - agent_configs: Each Agent's activity config (activity, post frequency, stance, etc.)
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script (generic script in backend/scripts/)
    ...
    """
    try:
        # Script is in backend/scripts/ directory
...
# ============== Profile Generation Endpoint (Independent Use) ==============
...
def generate_profiles():
    ...
    try:
        ...
        if filtered.filtered_count == 0:
            return jsonify({
                "success": False,
                "error": "No qualifying entities found"
            }), 400
...
# ============== Simulation Run Control Endpoint ==============
    ...
    Request（JSON）：
        {
            ...
            "max_rounds": 100,                     // Optional: Max simulation rounds, used to truncate too long simulations
            "enable_graph_memory_update": false,   // Optional: Whether to dynamically update Agent activities to Zep knowledge graph memory
            "force": false                         // Optional: Force restart (will stop running simulation and clean logs)
        }

    About force parameter:
        - When enabled, if simulation is running or completed, will first stop and clean running logs
        - Cleaned content includes: run_state.json, actions.jsonl, simulation.log, etc.
        - Won't clean config files (simulation_config.json) and profile files
        - Suitable for scenarios where you need to re-run simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activities in simulation (posting, commenting, liking, etc.) will be updated to Zep knowledge graph in real-time
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                logger.info(f"Simulation {simulation_id} preparation complete, reset to ready (original status: {state.status.value})")
...
def get_run_status_detail(simulation_id: str):
    """
    Get simulation detailed status (includes all actions)
    ...
    try:
        ...
        # recent_actions only shows latest round content from both platforms
...
def get_simulation_posts(simulation_id: str):
    ...
    Returns post list (from SQLite database)
    """
    try:
        platform = request.args.get('platform', 'reddit')
...
def get_simulation_comments(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get comment: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be running (after completing simulation loop enters wait for command mode)
    ...
    Returns (when platform not specified, dual-platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    Returns (when platform specified):
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation is completed and in command waiting mode."
            }), 400
    ...
    Request（JSON）：
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // If not specified, use default value
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by each item's platform)
                                               // When not specified: dual-platform simulates each Agent interviewing both platforms
...
def interview_all_agents():
    ...
    Request（JSON）：
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What is your overall view on this?",  // Required, interview question (all Agents use same question)
    ...
    Request（JSON）：
        {
            ...
            "platform": "reddit",          // Optional, platform type (reddit/twitter)
                                           // When not specified, returns all history from both platforms
...
def close_simulation_env():
    ...
    Send close environment command to simulation, let it exit command waiting mode gracefully.

    Note: This is different from /stop interface, /stop will forcibly terminate process,
    while this interface will let simulation gracefully close environment and exit.
