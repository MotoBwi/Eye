"""Zep Graph Paging Read Tool.

Zep's node/edge list interface uses UUID cursor pagination,
this module encapsulates auto-pagination logic (with per-page retry), transparently returning complete list to caller.
"""
...
logger = get_logger('godseye.zep_paging')
...
def fetch_all_nodes(
    ...
    """Paginate fetch graph nodes, up to max_items (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate fetch all graph edges, return complete list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
