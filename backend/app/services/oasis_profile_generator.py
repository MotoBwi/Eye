...
1. Call Zep retrieval to enrich node info
...
logger = get_logger('godseye.oasis_profile')
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name,  # OASIS requires username without underscore
            "name": self.name,
            "bio": self.bio,
...
class OasisProfileGenerator:
    ...
    Convert entities from Zep graph to Agent Profile format required by OASIS simulation
    ...
    # Group/Organization entity types (need to generate group representative persona)
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        if use_llm:
            ...
            # Use rule to generate basic persona
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            # Parallel execute edges and nodes search
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determine if is personal types entity"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if is group/institution types entity"""
    ...
    def _generate_profile_with_llm(
        ...
        Distinguish according to entity types:
        - Personal entity: generate specific character persona
        - Group/institution entity: generate representative account persona
        """
        ...
        # Try multiple times to generate until success or reach max retry count
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Do not set max_tokens, let LLM freely generate
                )
        ...
        logger.warning(f"LLM persona generation failed ({max_attempts} attempts): {last_error}, using rules to generate")
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if last quote has no comma or closing bracket, string may be truncated
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Respond in English. Generate detailed, realistic personas for opinion simulation, maximizing realism. Must return valid JSON format, all string values cannot contain unescaped newlines."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        """Build detailed persona prompt for personal entity"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media user persona for the entity, maximizing realism.
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Personal memory (important part of persona - describe this individual's connection to the event and their existing actions/reactions in the event)
3. age: Age number (must be integer)
4. gender: Gender, must be in English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP)
6. country: Country (use English, e.g., "India", "USA")
...
- Respond in English (only gender field must be "male" or "female")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed social media account settings for organization/group entity, maximizing realism.
...
1. bio: Official account bio, 200 characters, professional
2. persona: Detailed account setting description (2000 characters of plain text), must include:
   - Basic organization info (official name, organization nature, founding background, main functions)
   ...
   - Content characteristics (content types, posting frequency, active time period)
   - Stance (official stance on core topics, how to handle controversies)
   - Special notes (represented group profile, operation habits)
   - Organization memory (important part - describe the organization's connection to the event and its existing actions/reactions in the event)
3. age: Must be 30 (virtual age for organization accounts)
4. gender: Must be "other" (organization accounts use "other")
...
- Respond in English (only gender field must be "other")
...
- Organization account speech must match its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        # Generate different persona based on entity types
    ...
    def generate_profiles_from_entities(
        ...
        # Helper function for real-time write to file
        ...
        print(f"\n{'='*60}")
        print(f"Starting Generate Agent persona - {total} entities total, parallel count: {parallel_count}")
        print(f"{'='*60}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    if error:
                        logger.warning(f"[{current}/{total}] {entity.name} Use backup persona: {error}")
                    else:
                        logger.info(f"[{current}/{total}] Successfully generated persona: {entity.name} ({entity_type})")
                        
                except Exception as e:
                    logger.error(f"Processing entity {entity.name}  occurred exception: {str(e)}")
                    with lock:
                        completed_count[0] += 1
                    ...
                    # Real-time write to file (even as backup persona)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Only output to console (avoid duplication, logger does not output complete content)
        print(output)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile as CSV format (complies with OASIS official requirements)
