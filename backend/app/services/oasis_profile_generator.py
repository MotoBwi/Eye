...
class OasisProfileGenerator:
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 7. 完全失败，Returns基础结构
        logger.warning(f"JSON修复失败，Returns基础结构")
        return {
...
- Organization account speech must match its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            parallel_count: 并行生成数量，Default5
        ...
        logger.info(f"开始并行生成 {total} 个Agent人设（并Row count: {parallel_count}）...")
        print(f"\n{'='*60}")
        print(f"开始Generate Agent persona - 共 {total} 个实体，并Row count: {parallel_count}")
        print(f"{'='*60}\n")
    ...
    def save_profiles(
        ...
        Args:
            profiles: Profile列表
            file_path: 文件路径
            platform: 平台Types ("reddit" 或 "twitter")
        """
        if platform == "twitter":
