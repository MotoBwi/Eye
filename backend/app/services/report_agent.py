...
class ReportLogger:
    ...
    每行是一个完整的 JSON 对象，包含时间戳、动作Types、详细内容等。
    """
    ...
    def log(
        ...
        Args:
            action: 动作Types，如 'start', 'tool_call', 'llm_response', 'section_complete' 等
    ...
    def log_tool_result(
        ...
        self.log(
            ...
            details={
                ...
                "message": f"工具 {tool_name} Returns结果"
            }
        )
...
Default在Twitter和Reddit两个平台同时采访，获取更全面的观点。
...
【模拟世界规模】
- 参与模拟的Entity count: {total_nodes}
- 实体间产生的关系数量: {total_edges}
- 实体Types分布: {entity_types}
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "insight_forge": {
                ...
                "parameters": {
                    "query": "你想深入分析的问题或话题",
                    "report_context": "当前报告章节的上下文（Optional，有助于生成更精准的子问题）"
                }
            },
            "panorama_search": {
                ...
                "parameters": {
                    "query": "搜索查询，用于相关性排序",
                    "include_expired": "是否包含过期/历史内容（DefaultTrue）"
                }
            },
            "quick_search": {
                "name": "quick_search",
                ...
                "parameters": {
                    "query": "搜索查询字符串",
                    "limit": "Returns结果数量（Optional，Default10）"
                }
            },
            "interview_agents": {
    ...
    def plan_outline(
        ...
        try:
            ...
            # ReturnsDefault大纲（3个章节，作为fallback）
    ...
    def _generate_section_react(
        ...
        3. Observation（观察）- 分析工具Returns结果
        4. 重复直到信息足够或达到最大次数
        ...
        for iteration in range(max_iterations):
            ...
            # 检查 LLM Returns是否为 None（API 异常或内容为空）
            if response is None:
                logger.warning(f"章节 {section.title} 第 {iteration + 1} 次迭代: LLM Returns None")
                # 如果还有迭代次数，添加消息并重试
                if iteration < max_iterations - 1:
                    messages.append({"role": "assistant", "content": "（响应为空）"})
                    messages.append({"role": "user", "content": "请继续生成内容。"})
                    continue
                # 最后一次迭代也Returns None，跳出循环进入强制收尾
                break
        ...
        # 检查强制收尾时 LLM Returns是否为 None
        if response is None:
            logger.error(f"章节 {section.title} 强制收尾时 LLM Returns None，使用Default错误提示")
            final_answer = f"（本章节生成失败：LLM Returns空响应，请稍后重试）"
    ...
    def generate_report(
        ...
        Args:
            ...
            report_id: 报告ID（Optional，如果不传则自动生成）
