...
logger = get_logger('godseye.simulation_runner')
...
class SimulationRunState:
    ...
    # Platform completion status (detected by checking for simulation_end event in actions.jsonl)
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Contains detailed recent action info"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    2. Parse run log, record each Agent's actions
    ...
    def start_simulation(
        ...
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError("Must provide graph_id when graph memory update is enabled")
        ...
        try:
            ...
            # If max rounds specified, add to command line parameters
            if max_rounds is not None and max_rounds > 0:
            ...
            # Use start_new_session=True to create new process group, ensure can terminate all child processes via os.killpg
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensure can terminate all related processes when server shuts down
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # After process ends, read log one final time
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform running, check that platform only
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Overall rounds take max of both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time take max of both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Check if platform is enabled by checking if corresponding actions.jsonl file exists
        ...
        # Check which platforms are enabled (by checking if files exist)
    ...
    def get_all_actions(
        ...
        Returns:
            Complete action list (sorted by timestamp, new first)
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean simulation run logs (for forced restart of simulation)
        ...
        # Files to delete list (including database files)
        files_to_delete = [
            "run_state.json",
            "simulation.log",
            "stdout.log",
        ...
        # Directories to delete list (containing action logs)
    ...
    def cleanup_all_simulations(cls):
        ...
        Called when server shuts down, ensure all child processes are terminated
        """
            ...
            try:
                if process.poll() is None:  # Process still running
                    ...
                    try:
                        ...
                        # Process may already not exist, try terminating directly
                        try:
                            process.terminate()
    ...
    def register_cleanup(cls):
        ...
        Called when Flask app starts, ensure all simulation processes cleaned up when server shuts down
        """
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (actual running app process)
        # WERKZEUG_RUN_MAIN=true means reloader subprocess
        # If not debug mode, this environment variable doesn't exist, also need to register
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If original handler not callable (like SIG_DFL), use default behavior
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (Optional)
                ...
                - None: for dual-platform simulation, interview both platforms, return integrated result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (Optional, will be overridden by each interview item's platform)
                ...
                - None: for dual-platform simulation, each Agent interviewed on both platforms
            timeout: Timeout (seconds)
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation, make it exit command-waiting mode gracefully
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (wait response timeout, environment may be closing)"
            }
