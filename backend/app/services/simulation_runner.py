...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # 最大模拟轮数（Optional，用于截断过长的模拟）
        ...
        Args:
            ...
            max_rounds: 最大模拟轮数（Optional，用于截断过长的模拟）
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # 获取平台：优先使用记录中的 platform，否则使用Default平台
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(sim_dir):
            return {"success": True, "message": "Simulation directory does not exist，无需清理"}
    ...
    def register_cleanup(cls):
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # 如果原处理器不可调用（如 SIG_DFL），则使用Default行为
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: 指定平台（Optional）
                ...
                - "reddit": 只采访Reddit平台
                - None: 双平台模拟时同时采访两个平台，Returns整合结果
            timeout: 超时时间（秒）
        ...
        Raises:
            ValueError: Simulation does not exist或环境未运行
            TimeoutError: 等待响应超时
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: 模拟ID
            interviews: 采访列表，每个元素包含 {"agent_id": int, "prompt": str, "platform": str(Optional)}
            platform: Default平台（Optional，会被每个采访项的platform覆盖）
    ...
    def get_interview_history(
        ...
        Args:
            simulation_id: 模拟ID
            platform: 平台Types（reddit/twitter/None）
                - "reddit": 只获取Reddit平台的历史
                - "twitter": 只获取Twitter平台的历史
                - None: 获取两个平台的所有历史
            agent_id: 指定Agent ID（Optional，只获取该Agent的历史）
            limit: 每个平台Returns数量限制
