"""
Smart Simulation Config Generator
...
Adopt step-by-step generation strategy to avoid failure from generating too long content at once：
...
logger = get_logger('godseye.simulation_config')

# Chinese daily schedule config (Beijing time)
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        ...
        "work": 0.7,       # Work hours medium activity
        "peak": 1.5,       # Evening peak
        "night": 0.5       # Decline at late night
    }
}
...
class AgentActivityConfig:
    ...
    # Active time period（24-hour，0-23）
    ...
    # Influence weight（Probability of its posts being seen by other Agents）
    influence_weight: float = 1.0


@dataclass  
class TimeSimulationConfig:
    """Time simulation config (based on daily habits)"""
    # Total simulation duration（Simulation hours）
    total_simulation_hours: int = 72  # Default 72 hours (3 days)
    
    # Time per round (Simulation minutes) - Default 60 minutes (1 hour), Speed up time flow
    ...
    # Low period（Dawn 0-5，Almost no activity）
...
class EventConfig:
    """Event config"""
    # Initial event（Trigger event at simulation start）
    initial_posts: List[Dict[str, Any]] = field(default_factory=list)
    
    # Timed event（Event triggered at specific time）
...
class PlatformConfig:
    ...
    # Viral spread threshold（How many interactions to trigger spread）
    viral_threshold: int = 10
    
    # Echo chamber effect intensity（Degree of similar viewpoints gathering）
...
class SimulationConfigGenerator:
    ...
    automatically generate optimal simulation parameters
    ...
    def generate_config(
        ...
        Args:
            ...
            enable_twitter: Enable Twitter or not
            enable_reddit: Enable Reddit or not
        ...
        """
        logger.info(f"Start smart generate simulation config: simulation_id={simulation_id}, entity_count={len(entities)}")
        
        # Calculate total steps
        num_batches = math.ceil(len(entities) / self.AGENTS_PER_BATCH)
        total_steps = 3 + num_batches  # Time config + Event config + N batches + Platform config
        current_step = 0
        ...
        # ========== Assign poster Agent for initial posts ==========
        logger.info("Assigning suitable poster Agent for initial posts...")
        event_config = self._assign_initial_post_agents(event_config, all_agent_configs)
        assigned_count = len([p for p in event_config.initial_posts if p.get("poster_agent_id") is not None])
        reasoning_parts.append(f"Initial post assignment: {assigned_count} posts assigned poster")
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # 500 chars margin
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    temperature=0.7 - (attempt * 0.1)  # Lower temp on each retry
                    # No max_tokens, let LLM decide
                )
...
### Basic Principles (for reference, adjust based on event and participant group):
- Follow typical daily routines for the target audience
- Dawn 0-5: Almost no activity (activity coefficient 0.05)
...
- General pattern: late night low, morning gradual increase, work hours medium, evening peak
- **Important**: The following example values are for reference only, adjust specific times based on event nature and participant group characteristics
  - Example: Student group peak may be 21-23; media active all day; official institutions only during work hours
  - Example: Breaking news may cause late night discussions, off_peak_hours can be appropriately shortened
...
- off_peak_hours (int array): Low period, usually late night
...
- reasoning (string): Brief explanation of why this configuration"""
        ...
        system_prompt = "You are a social media simulation expert. Respond in English. Returns pure JSON format, time config should follow typical daily routines."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time config (based on daily routine)"""
        return {
            "total_simulation_hours": 72,
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time config result, validate agents_per_hour does not exceed total agents"""
        # Get original values
...
**Important**: poster_type must be selected from the "available entity types" above so that initial posts can be assigned to the appropriate Agent for posting.
...
{{
    ...
    "reasoning": "<brief explanation>"
        ...
        system_prompt = "You are an opinion analysis expert. Respond in English. Returns pure JSON format. Note: poster_type must exactly match available entity Types."
    ...
    def _assign_initial_post_agents(
        ...
        """
        Assign suitable poster Agent for initial posts
        ...
        # Type aliases (handle different formats LLM might output)
        type_aliases = {
...
- **Follow typical daily routines**: Dawn 0-5 almost no activity, Evening 19-22 most active
- **Official institutions** (University/GovernmentAgency): Low activity (0.1-0.3), work hours (9-17), slow response (60-240 minutes), high influence (2.5-3.0)
- **Media** (MediaOutlet): Medium activity (0.4-0.6), all-day activity (8-23), fast response (5-30 minutes), high influence (2.0-2.5)
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Respond in English. Returns pure JSON, config should follow typical daily routines."
        
        try:
            result = self._call_llm_with_retry(prompt, system_prompt)
            llm_configs = {cfg["agent_id"]: cfg for cfg in result.get("agent_configs", [])}
        except Exception as e:
            logger.warning(f"Agent batch LLM generation failed: {e}, using rule generation")
            llm_configs = {}
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent config based on rules (daily routine)"""
