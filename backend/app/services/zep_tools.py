...
logger = get_logger('godseye.zep_tools')
...
class InsightForgeResult:
    ...
    Contains multiple sub-query results and comprehensive analysis
    """
    query: str
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### 【Currently Valid Facts】(Simulation Result Original)")
...
class AgentInterview:
    ...
    def to_text(self) -> str:
        ...
        # Show complete agent_bio, not truncated
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter garbage content containing question numbers (question1-9)
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate overly long content (by sentence, not hard truncate)
...
class InterviewResult:
    """
    Interview results
    Contains multiple simulated Agent interview answers
    """
...
class ZepToolsService:
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Calculate text match score with query"""
            if not text:
                return 0
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Gets all graph edges, then filters to those related to the specified node
    ...
    def get_simulation_context(
        ...
        Args:
            ...
            limit: Limit for each type of info
    ...
    def insight_forge(
        ...
        Most powerful hybrid retrieval function, automatically decomposes questions and multi-dimensional retrieval:
...
Return JSON format sub-question list."""
    ...
    def panorama_search(
        ...
        This tool is suitable for understanding the full picture of events and tracking evolution.
    ...
    def interview_agents(
        ...
        4. Call /api/simulation/interview/batch interface for real interviews (dual-platform simultaneous interviews)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "4. Answer each question one by one, each answer starting with 'Question X:' (X is the question number)\n"
            "5. Separate each answer with a blank line\n"
            "6. Answers should have substance, each question should be answered in at least 2-3 sentences\n\n"
        )
        ...
        try:
            ...
            for agent_idx in selected_indices:
                interviews_request.append({
                    ...
                    # Do not specify platform, API will interview on both twitter and reddit
                })
            ...
            if not api_result.get("success", False):
                ...
                logger.warning(f"Interview API returns failed: {error_msg}")
                result.summary = f"Interview API call failed: {error_msg}. Please check OASIS simulation environment status."
                return result
            
            # Step 5: Parse API returns, build AgentInterview object
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean possible JSON tool call wrapper
                ...
                # Strategy 2 (supplement): Long text within properly paired Chinese quotes 「」
                if not key_quotes:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean Agent response from JSON tool call wrapper, extract actual content"""
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: Selected agent's complete info list
                - selected_indices: Selected agent's index list (for API call)
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable agents for interview from the simulation agent list based on interview requirements.

Selection criteria:
1. Agent's identity/profession should be relevant to the interview topic
2. Agent may have unique or valuable perspectives
...
Candidate agent list ({len(agent_summaries)} total):
{json.dumps(agent_summaries, ensure_ascii=False, indent=2)}

Please select up to {max_agents} most suitable agents for interview and explain the selection reason."""
        ...
        try:
            ...
            # Get selected agent's complete info
            selected_agents = []
            valid_indices = []
            ...
            logger.warning(f"LLM select Agent failed, using default selection: {e}")
...
5. Each question should be concise, within 50 words
6. Ask directly, do not include background explanations or prefixes

Returns JSON format: {"questions": ["question1", "question2", ...]}"""
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are a professional news editor. Please generate an interview summary based on responses from multiple interviewees. Respond in English.

Summary requirements:
1. Summarize the main viewpoints of each party
2. Point out consensus and differences in opinions
...
- You can use **bold** for keywords, but do not use other Markdown syntax"""
...
Please generate an interview summary."""
        ...
        try:
            ...
            return f"Interviewed {len(interviews)} interviewees: " + ", ".join([i.agent_name for i in interviews])
